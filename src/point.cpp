...
void
Pos::on_btn_pago_efectivo_clicked ()
{
  if (total_vcarrito != 0 && spin_ingreso->get_value() >= total_vcarrito )
    {
      ...
      true;
        ...
        //               << "\n\n"
        //               << "--------------------------------\n";
      ticket << std::left << std::setw (20) << "Articulo" << std::setw (5) << "Cnt." << std::setw (5) << "P.U." << "T.\n"
                    << "--------------------------------\n";
          ...
          for (auto row_prod : m_refTreeModel_prod->children ())
            {
              if (row[m_Columns_venta.sku] == row_prod[m_Columns_prod.sku])
                {
                  int diff = row_prod[m_Columns_prod.piezas]
                             - row[m_Columns_venta.cantidad];
                  if (diff >= 0)
                    db->command (
                        "UPDATE producto SET piezas = " + std::to_string (diff)
                        + " WHERE sku = "
                        + std::to_string (row_prod[m_Columns_prod.sku]) + ";");
      ...
      if (StartDoc (printerDC, &di) > 0)
        {
          if (StartPage (printerDC) > 0)
            {
              // Imprimir el texto
              TextOut (printerDC, 100, 100, ticket.str ().c_str (),
                       ticket.str ().length ());
      ...
      db->command (
          "INSERT INTO venta VALUES (" + std::to_string (id) + ",'" + tipo
          + "', " + std::to_string (total_vcarrito) + ", "
          + spin_ingreso->get_text () + " , "
          + lbl_cambio->get_text ().substr (1, lbl_cambio->get_text ().size ())
          + ",'" + folio_tarjetaa.str ()
          + "' , datetime('now','localtime') , '" + ss.str () + "');");
      ...
      row_reporte[m_Columns_reporte.ingreso]
          = (float)spin_ingreso->get_value ();
      row_reporte[m_Columns_reporte.cambio]
          = (float)spin_ingreso->get_value () - total_vcarrito;
      row_reporte[m_Columns_reporte.folio] = folio_tarjetaa.str ();
      row_reporte[m_Columns_reporte.fecha]
          ...
          if (row[m_Columns_prod.sku] == std::stoll (ety_barras->get_text ()))
            {
              for (auto row_venta : ModelCarroVenta->children ())
                {
                  if (row_venta[m_Columns_venta.sku]
                      == row[m_Columns_prod.sku])
                    {
                      row_venta[m_Columns_venta.cantidad]
                          = row_venta[m_Columns_venta.cantidad].
                            operator std::size_t ()
                            + 1;
                      row_venta[m_Columns_venta.precio_t]
                          = row_venta[m_Columns_venta.cantidad].
                            operator std::size_t ()
              ...
              row_venta[m_Columns_venta.sku]
                  = row[m_Columns_prod.sku].operator long long ();
              row_venta[m_Columns_venta.cantidad] = 1;
              row_venta[m_Columns_venta.nombre]
                  = row[m_Columns_prod.nombre].operator Glib::ustring ();
              row_venta[m_Columns_venta.precio_u]
                  = row[m_Columns_prod.precio_u].operator float ();
              row_venta[m_Columns_venta.precio_t]
                  = row[m_Columns_prod.precio_u].operator float ();
...
Pos::on_spin_ingreso_changed ()
{
  ...
  if (std::stof (ss.str ()) < 0)
    {
      lbl_cambio->set_markup ("$<span font_desc='50' foreground='red'>"
                              + ss.str () + "</span>");
      btn_pago_efectivo->set_sensitive (false);
    }
  else
    {
      lbl_cambio->set_markup ("$<span font_desc='50'>" + ss.str ()
                              + "</span>");
...
bool
Pos::on_spin_ingreso_activate (guint keyval, guint, Gdk::ModifierType state)
{
...
void
Pos::on_btn_pago_tarjeta_clicked ()
{
...
bool
Pos::add_match_arcticulo (const Gtk::TreeModel::iterator &iter)
{
  std::stringstream ss;
  ...
  for (auto row_venta : ModelCarroVenta->children ())
    {
      if (row_venta[m_Columns_venta.sku]
          == iter->get_value (m_Columns_prod.sku))
        {
          row_venta[m_Columns_venta.cantidad]
              = row_venta[m_Columns_venta.cantidad].operator std::size_t ()
                + 1;
          row_venta[m_Columns_venta.precio_t]
              = row_venta[m_Columns_venta.cantidad].operator std::size_t ()
                * row_venta[m_Columns_venta.precio_u].operator float ();
          total_vcarrito
              += row_venta[m_Columns_venta.precio_u].operator float ();
          ety_barras->set_text ("");
          ss << std::fixed << std::setprecision (2) << total_vcarrito;
          lbl_precio_total->set_markup ("$<span font_desc='50'>" + ss.str ()
                                        + "</span>");
          on_spin_ingreso_changed ();
          return true;
        }
  ...
  row_venta[m_Columns_venta.precio_u]
      = iter->get_value (m_Columns_prod.precio_u);
  row_venta[m_Columns_venta.precio_t]
      = iter->get_value (m_Columns_prod.precio_u);
